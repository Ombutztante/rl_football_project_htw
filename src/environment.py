    ...
    def step(self, action):
        ...
        if action == 4:  # shoot / pass
            ...
            if self.level == 4 and (new_x, new_y) in self.obstacle_cells:
                reward += config.REWARD_HIT_OBSTACLE  # -2, blocked, agent stays in place
            else:
            ...
            # Ball pickup — blocked while pass is in flight; reward fires only once per episode
            ...
            if self.level == 4 and self.has_ball and not self.bypass_rewarded:
                if (self.agent_pos[0] > config.OBSTACLE_X and
                    ...
                    reward += config.REWARD_BYPASS_OBSTACLE  # +2
